...
| 7-E Lateral Movement | ssh (OpenSSH 9.5) | credential spray to trip detection: loop `ssh -o PreferredAuthentications=password -o PubkeyAuthentication=no svc-backup@10.10.10.10` through the tunnel (T1110) | **5763** — sshd brute-force ticket, level 10, source correctly attributed to win-ep |
| 7-E Lateral Movement | ssh (OpenSSH 9.5) | authenticate with the stolen key: `ssh -i stolen_key svc-backup@10.10.10.10 "id; hostname; ls /opt/homelab-mdr/..."` — valid-account access (T1021.004 + T1078), then reads detection config (T1005) | *in progress* — login succeeds (`Accepted publickey` in auth.log) but does not yet reach Wazuh; auditd not yet capturing the non-interactive commands |
...
- **Phase 7 — Adversary emulation (GentleKiller kill-chain) — mostly implemented:** rather than a single ransomware payload, Phase 7 reconstructs the *full intrusion lifecycle* of The Gentlemen / GentleKiller — the #2 RaaS of 2026 (ESET, June 2026) — as an 8-stage attack chain, each stage emulated with professional tooling and validated attack → detection → SOAR → TheHive. **Done:** 7-A initial access (CVE-2024-55591 auth-bypass), 7-B fileless in-memory execution → C2, 7-C persistence (writer-agnostic ASEP), 7-D BYOVD EDR-killer with the driver-load↔kill **correlation** (the flagship build), discovery, credential access (LSASS), and DNS-C2 rules — plus two original layers the real victims lacked: **L4**, a resilient "surviving channel" that tells a killed agent apart from a powered-off host (endpoint-silence + off-host network-alive), and **L0**, a SIEM self-health monitor that catches the detection engine dying silently. **In progress:** 7-E lateral movement (win-ep → siem: SSH key stolen over the C2 channel → Ligolo-ng tunnel → SSH spray then valid-account login with the stolen key — the chain runs end-to-end, with the SIEM correctly attributing the source to the compromised endpoint; two detection gaps are being closed). **Remaining:** 7-impact (mass encryption + shadow-copy deletion). The design centers on *behavioral* detection — the strongest signal against an actor that swaps 8 BYOVD driver variants and writes its tooling in Go. Professional tooling: **Sliver C2** (in-memory implant, C2, key theft over the encrypted channel), **Ligolo-ng** (userland-TUN tunnelling — the pivot that let the external attacker reach the SIEM through the compromised endpoint), **Atomic Red Team** (T1490/T1489 impact tests), **Nuclei** (edge probing); planned for impact: **Infection Monkey** (safe reversible ransomware payload). Full design: `docs/phase7-attack-scenario.md`.
