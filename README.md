...
- **Phase 7 — Adversary emulation (GentleKiller kill-chain) — mostly implemented:** rather than a single ransomware payload, Phase 7 reconstructs the *full intrusion lifecycle* of The Gentlemen / GentleKiller — the #2 RaaS of 2026 (ESET, June 2026) — as an 8-stage attack chain, each stage emulated with professional tooling and validated attack → detection → SOAR → TheHive. **Done:** 7-A initial access (CVE-2024-55591 auth-bypass), 7-B fileless in-memory execution → C2, 7-C persistence (writer-agnostic ASEP), 7-D BYOVD EDR-killer with the driver-load↔kill **correlation** (the flagship build), discovery, credential access (LSASS), and DNS-C2 rules — plus two original layers the real victims lacked: **L4**, a resilient "surviving channel" that tells a killed agent apart from a powered-off host (endpoint-silence + off-host network-alive), and **L0**, a SIEM self-health monitor that catches the detection engine dying silently. **Done (7-E lateral movement):** the SSH key stolen over the Sliver C2 channel → Ligolo-ng tunnel → valid-account login to the SIEM, proven end-to-end through the pivot — the SIEM attributes the source to the compromised endpoint (win-ep), not the attacker behind it. Detection is allowlist-based (any publickey login to the SIEM from a non-jump source, 100210/100211, detect-only on the management plane) and the SOAR correlates it cross-host with the source's recent high-severity activity into one Critical lateral-movement ticket — the "raise attention on the host" pattern (auditd non-interactive-command capture remains). **Remaining:** 7-impact (mass encryption + shadow-copy deletion). The design centers on *behavioral* detection — the strongest signal against an actor that swaps 8 BYOVD driver variants and writes its tooling in Go. Professional tooling: **Sliver C2** (in-memory implant, C2, key theft over the encrypted channel), **Ligolo-ng** (userland-TUN tunnelling — the pivot that let the external attacker reach the SIEM through the compromised endpoint), **Atomic Red Team** (T1490/T1489 impact tests), **Nuclei** (edge probing); planned for impact: **Infection Monkey** (safe reversible ransomware payload). Full design: `docs/phase7-attack-scenario.md`.
