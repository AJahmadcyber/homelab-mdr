...
| 7-E Lateral Movement | ssh (OpenSSH 9.5) **over the Ligolo tunnel** | authenticate with the stolen key: `ssh -i stolen_key svc-backup@10.10.10.10 "id; hostname; ls /opt/homelab-mdr/..."` — valid-account access through the tunnel (T1021.004 + T1078), then reads detection config (T1005) | *in progress* — login succeeds (`Accepted publickey` in auth.log) but does not yet reach Wazuh; auditd not yet capturing the non-interactive commands |
